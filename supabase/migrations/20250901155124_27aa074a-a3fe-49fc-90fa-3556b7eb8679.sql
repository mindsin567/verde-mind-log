-- First, clear all existing data since integer user_ids cannot be mapped to UUID auth users
TRUNCATE public.moodlogs CASCADE;
TRUNCATE public.diaryentries CASCADE;
TRUNCATE public.chatmessages CASCADE;
TRUNCATE public.aisummaries CASCADE;
TRUNCATE public.airecommendations CASCADE;

-- Add auth_id to users table and modify the structure
...
ALTER TABLE public.moodlogs ADD COLUMN user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE;

-- Update diaryentries table
ALTER TABLE public.diaryentries DROP COLUMN user_id;
ALTER TABLE public.diaryentries ADD COLUMN user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE;

-- Update chatmessages table
ALTER TABLE public.chatmessages DROP COLUMN user_id;
ALTER TABLE public.chatmessages ADD COLUMN user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE;

-- Update aisummaries table
ALTER TABLE public.aisummaries DROP COLUMN user_id;
ALTER TABLE public.aisummaries ADD COLUMN user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE;

-- Update airecommendations table
ALTER TABLE public.airecommendations DROP COLUMN user_id;  
ALTER TABLE public.airecommendations ADD COLUMN user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE;
